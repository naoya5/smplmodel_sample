...
def create_sample_data():
    ...
    with open(gaze_csv_path, "w", encoding="utf-8") as f:
        ...
        for i in range(1000):
            ...
            # 信頼度（時々低くなる）
            confidence = (
                0.95 if np.random.random() > 0.1 else np.random.uniform(0.6, 0.8)
            )

            f.write(
                f"{timestamp:.3f},{gaze_x:.6f},{gaze_y:.6f},{gaze_z:.6f},"
...
def example_basic_analysis():
    ...
    print(
        f"   時間範囲: {gaze_data[0].timestamp:.2f}s - {gaze_data[-1].timestamp:.2f}s"
    )
...
def example_attention_hotspots(temporal_distribution, mesh_processor):
    ...
    vertex_avg_attention = {
        vertex_idx: np.mean(probs) for vertex_idx, probs in vertex_attention.items()
    }

    # 2. トップ10の注目頂点を特定
    print("\n2. 最も注目された頂点トップ10:")
    sorted_vertices = sorted(
        vertex_avg_attention.items(), key=lambda x: x[1], reverse=True
    )

    for i, (vertex_idx, avg_attention) in enumerate(sorted_vertices[:10]):
        vertex_pos = mesh_processor.vertices[vertex_idx]
        print(
            f"   {i + 1:2d}. 頂点{vertex_idx:4d}: 平均注目度={avg_attention:.4f}, "
    ...
    for timestamp in sorted(temporal_distribution.keys()):
        ...
        prob = next(
            (vp.probability for vp in vertex_probs if vp.vertex_index == top_vertex),
            0.0,
        )
...
def example_temporal_patterns(temporal_distribution):
    ...
    for timestamp in timestamps:
        ...
        max_attention = (
            max(vp.probability for vp in vertex_probs) if vertex_probs else 0
        )
    ...
    print(
        f"   総注目度: 平均={np.mean(total_attentions):.4f}, "
        f"標準偏差={np.std(total_attentions):.4f}"
    )
    print(
        f"   最大注目度: 平均={np.mean(max_attentions):.4f}, "
        f"標準偏差={np.std(max_attentions):.4f}"
    )
    print(
        f"   活性頂点数: 平均={np.mean(active_vertices):.1f}, "
    ...
    with open(timeseries_csv, "w", encoding="utf-8") as f:
        ...
        for data in time_series_data:
            f.write(
                f"{data['timestamp']:.3f},{data['total_attention']:.6f},"
    ...
    if significant_changes:
        ...
        with open(changes_csv, "w", encoding="utf-8") as f:
            ...
            for change in significant_changes:
                f.write(
                    f"{change['timestamp']:.3f},{change['change_magnitude']:.6f},"
