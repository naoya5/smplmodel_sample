...
class SpatialRelationCalculator:
    ...
    @staticmethod
    def point_to_line_distance(point: np.ndarray, line_origin: np.ndarray, 
    ...
    @staticmethod
    def calculate_gaze_angle(vertex: np.ndarray, gaze_origin: np.ndarray,
                           gaze_direction: np.ndarray, vertex_normal: np.ndarray) -> float:
        """
        視線と頂点法線の角度を計算
        ...
        # 視線方向と頂点方向の角度
        gaze_angle = np.arccos(np.clip(np.dot(gaze_direction, vertex_direction), -1.0, 1.0))
        
        # 視線方向と頂点法線の角度
        normal_angle = np.arccos(np.clip(np.dot(-gaze_direction, vertex_normal), -1.0, 1.0))
...
class GazeDistributionAnalyzer:
    ...
    def calculate_vertex_probabilities(self, gaze_points: List[GazePoint],
                                     timestamp: float) -> List[VertexGazeProbability]:
        """
        ...
        for vertex_idx, (vertex, normal) in enumerate(zip(vertices, vertex_normals)):
            ...
            for gaze in time_filtered_gazes:
                ...
                # 総合確率スコア
                combined_score = (distance_score * angle_score * 
                                temporal_weight * gaze.confidence)
    ...
    def analyze_temporal_distribution(self, gaze_points: List[GazePoint],
                                    time_step: float = 0.1) -> Dict[float, List[VertexGazeProbability]]:
        """
        時系列にわたる視線分布を解析
        ...
        while current_time <= max_time:
            vertex_probs = self.calculate_vertex_probabilities(gaze_points, current_time)
            temporal_distribution[current_time] = vertex_probs
            current_time += time_step
            
        return temporal_distribution
    
    def fit_gaussian_mixture_model(self, temporal_distribution: Dict[float, List[VertexGazeProbability]],
                                 n_components: int = 3) -> GaussianMixture:
        """
        視線分布データにガウス混合モデルを適用
...
class ResultExporter:
    ...
    @staticmethod
    def export_to_csv(temporal_distribution: Dict[float, List[VertexGazeProbability]],
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            # ヘッダー
            writer.writerow([
                'timestamp', 'vertex_index', 'probability',
                'distance_score', 'angle_score', 'temporal_weight'
            ])
    ...
    @staticmethod
    def export_to_json(temporal_distribution: Dict[float, List[VertexGazeProbability]],
                      output_path: Union[str, Path]) -> None:
        """
    ...
    @staticmethod
    def visualize_temporal_heatmap(temporal_distribution: Dict[float, List[VertexGazeProbability]],
        ...
        # 時刻ラベルの設定
        time_ticks = np.linspace(0, len(timestamps)-1, min(10, len(timestamps)), dtype=int)
...
def main():
    ...
    ResultExporter.export_to_csv(
        temporal_distribution, 
        output_dir / "gaze_distribution.csv"
    )
    
    # JSON出力
    ResultExporter.export_to_json(
        temporal_distribution,
        output_dir / "gaze_distribution.json"
    )
