...
class GazeResearchAnalyzer:
    ...
    def calculate_part_statistics(self, df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        """
        各部位の統計情報を計算
        ...
        value_columns = [col for col in df.columns if col.endswith('_value') and col != 'max_value']
        ...
        for part in part_names:
            ...
            if value_col in df.columns:
                ...
                stats[part] = {
                    ...
                    'time_with_gaze_seconds': float((values > 0).sum() * self.frame_duration),
                ...
                stats[part]['dominant_time_seconds'] = float(max_part_count * self.frame_duration)
                stats[part]['dominant_percentage'] = float(max_part_count / total_frames * 100)
    ...
    def save_results(self, output_dir: str, part_durations: Dict[str, float], 
                    transitions: List[Dict[str, Any]], stats: Dict[str, Dict[str, float]], 
        ...
        with open(duration_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for part, duration in sorted(part_durations.items(), key=lambda x: x[1], reverse=True):
        ...
        with open(transitions_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['transition_id', 'frame', 'time_seconds', 'from_part', 'to_part'])
        ...
        # 5. サマリーレポートを生成
        self._generate_summary_report(output_dir, part_durations, transitions, stats, timeline)
        
    def _generate_summary_report(self, output_dir: str, part_durations: Dict[str, float], 
                                transitions: List[Dict[str, Any]], stats: Dict[str, Dict[str, float]], 
                                timeline: Dict[str, Any]) -> None:
        """
        サマリーレポートを生成
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            sorted_durations = sorted(part_durations.items(), key=lambda x: x[1], reverse=True)[:10]
            ...
            for trans in transitions[:10]:
                f.write(f"- {trans['time_seconds']:.3f}秒: {trans['from_part']} → {trans['to_part']}\n")
